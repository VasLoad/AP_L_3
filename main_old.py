...
class Application(tk.Tk):
    ...
    def go(self, route: Route):
        frame: Optional[tk.Frame] = self.__frames.get(route)
    ...
    def __configure_style(self):
        self.__style.configure(MAIN_TFRAME_STYLE_NAME)
    ...
    def __build_frames(self, raw_frames: dict) -> dict[Route, BaseFrame]:
        ...
        for route, FrameClass in raw_frames.items():
            frame = FrameClass(parent=self.__container, controller=self)
